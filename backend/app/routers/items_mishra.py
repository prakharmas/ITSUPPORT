...
def read_items(
    ...
    created_last_days: Optional[int] = Query(None),   # ? Added
    done_last_days: Optional[int] = Query(None),      # ? Added
    ...
    if created_last_days:
        query = query.filter(WorkItem.created_at >= now - timedelta(days=created_last_days))

    if done_last_days:
        query = query.filter(
            WorkItem.status == 'done',
            WorkItem.completed_at.isnot(None),
            WorkItem.completed_at >= now - timedelta(days=done_last_days)
        )        
