...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const openTickets = items.filter(item => item.status === 'backlog')
      ...
      const rcaPending = items.filter(item => 
        item.status !== 'done' && 
        (item.rca_status === 'pending' || item.rca_status === 'in_progress')
      )
      
      const solutionPending = items.filter(item => 
        item.status !== 'done' && 
        (item.solution_status === 'pending' || item.solution_status === 'in_progress')
      )
      ...
      setStats({
        ...
        solutionPending: solutionPending.length,
  ...
  const openDetailModal = (cardKey, cardLabel) => {
    ...
    if (cardKey === 'myItems') {
      ...
      filteredItems = allItems.filter(item => item.status === 'backlog')
    ...
    } else if (cardKey === 'rcaPending') {
      filteredItems = allItems.filter(item => 
        item.status !== 'done' && 
        (item.rca_status === 'pending' || item.rca_status === 'in_progress')
      )
    } else if (cardKey === 'solutionPending') {
      filteredItems = allItems.filter(item => 
        item.status !== 'done' && 
        (item.solution_status === 'pending' || item.solution_status === 'in_progress')
      )
    } else if (cardKey === 'recurring') {
      filteredItems = allItems.filter(item => 
        item.is_recurring === true
      )
    }
